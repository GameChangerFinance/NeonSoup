...
export function summarizeSwapBookPolicyFilters(
  ...
  const normalized = normalizeSwapBook({ offers, offerAsset, receiveAsset, payUp: false, payUpBps: 0 });
...
export function quoteSwap({
  ...
  const normalized = normalizeSwapBook({ offers, offerAsset, receiveAsset, payUp, payUpBps });
