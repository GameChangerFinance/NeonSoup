...
export async function loadAddressTransactions(state: AppState, address: string, limit = 50): Promise<ChainTransaction[]> {
  return provider(state).getAddressTransactions(context(state), address, limit);
}
