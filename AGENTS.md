...
provider, wallet, GCScript, and core helpers so future simplified user-facing
...
- Future UIs should import reusable behavior from `src/core/index.ts`, not from
  `src/devtool/src/**`.
...
- Core must not import React, Bootstrap, CSS, devtool app state/hooks,
  ...
  APIs.
- Browser-only responsibilities belong in devtool adapters: popup/window
  behavior, return URL cleanup, `localStorage`, `history.replaceState`, URL
  param decoding, and wallet launch.
...
- The user-facing Frontend goal is a simplified, friendly, AMM-like DEX UX over
  the order book. Hide order-book complexity without pretending it is an AMM
  pool.
